...
int main(void)
{
  ...
  while (1)
  {
		if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_2)) // if the pin is HIGH 
		{
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, 0);
			lcd_puts(0, 0, (int8_t*)"RAIN DETECTED: Y");
			lcd_puts(1, 0, (int8_t*)"ROOF CLOSING:  Y");
		} 
		else if(!HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_2)){
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, 1);
			lcd_puts(0, 0, (int8_t*)"RAIN DETECTED: N");
			lcd_puts(1, 0, (int8_t*)"ROOF CLOSING:  N");
		}
    /* USER CODE END WHILE */
